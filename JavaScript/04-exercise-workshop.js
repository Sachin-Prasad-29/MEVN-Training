...
// find all workshops which have category "frontend"
const frontedCat = workshops.filter((item) => item.category === "frontend");
console.log(frontedCat);


// find all workshops in Bangalore
const locationBan = workshops.filter((item) => item.location.city === "Bangalore");
console.log(locationBan);


// find the first workshop which is available only in online mode
const onlineModeWorkShop = workshops.find((item) => item.modes.online);
console.log(onlineModeWorkShop);


// find the first workshop which is available only in both online and inPerson modes
const inPerson_online = workshops.find((item) => item.modes.online && item.modes.inPerson);
console.log(inPerson_online);


//get a list of workshop imageurl
const imageURL = workshops.map(i => i.imageUrl);
...
//     ...
// ]
const name_id_list = workshops.map(i => {
    return{
        name:i.name,
        id:i.id
    };
});
console.log(name_id_list);

// get the number of online workshops
const num_online_workshop = workshops.reduce((count,item)  => count + (item.modes.online ? 1: 0),0);
console.log(num_online_workshop);


// get the number of inPerson workshops
const num_inPerson_workshop = workshops.reduce((count,item)  => count + (item.modes.inPerson ? 1: 0),0);
console.log(num_inPerson_workshop);


// for all the frontend workshop
const all_frontEnd_workshop = workshops.filter(w => w.category === "frontend").map(w => w.name);
console.log(all_frontEnd_workshop);


const ans  = all_frontEnd_workshop.join(', ');
console.log(ans);
